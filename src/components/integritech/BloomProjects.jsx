...
import { ArrowUpRight, ArrowLeft, ArrowRight, Instagram } from 'lucide-react';
...
export const BloomProjects = ({ lang }) => {
  ...
  const websiteProjects = PROJECTS.filter((p) => p.categories.includes('Website'));
  const project = websiteProjects[currentIndex];

  const handlePrev = () => {
    setCurrentIndex((prev) => (prev === 0 ? websiteProjects.length - 1 : prev - 1));
  };

  const handleNext = () => {
    setCurrentIndex((prev) => (prev === websiteProjects.length - 1 ? 0 : prev + 1));
  };
                    ...
                    <div className="mt-6 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-semibold text-ink">
                          {currentIndex + 1} / {websiteProjects.length}
                        </span>
                        <div className="flex gap-1.5">
                          {websiteProjects.map((_, i) => (
                            <button
                              key={i}
                              type="button"
